...
int main(int argc, char **argv)
{
   ...
   {  
      vector<string> files = globVector(string(argv[1]) + "/*jpg");
	 ...
	 //cout << files[i] << endl;
         ...
         double t1 = 0;
         {
            ...
            t1 = getTime(); g_numberOfPoints = 0; g_numberOfAffinePoints = 0;
            detector.detectPyramidKeypoints(image);
            cout << "Number " << i << " Detected " << g_numberOfPoints << " keypoints and " << g_numberOfAffinePoints << " affine shapes in " << getTime()-t1 << " sec." << endl;

	        size_t found = files[i].rfind('/');
	        string name = files[i].substr(found+1, files[i].size() - found - 5);
	        //cout << string(argv[2]) + "/" + name << endl;
            ofstream out(string(argv[2]) + "/" + name);
